...
def ensure_path(path, remove=True):
    ...
    if os.path.exists(path):
        if remove and (
            basename.startswith("_") or input("{} exists, remove? ([y]/n): ".format(path)) != "n"
        ):
            shutil.rmtree(path)
            os.makedirs(path)
    else:
...
def mean_confidence_interval(data, confidence=0.95):
    a = 1.0 * np.array(data)
    stderr = stats.sem(a)
    h = stderr * stats.t.ppf((1 + confidence) / 2.0, len(a) - 1)
    return h
...
def config_inner_args(inner_args):
    ...
    inner_args["momentum"] = inner_args.get("momentum") or 0.0
    inner_args["weight_decay"] = inner_args.get("weight_decay") or 0.0
