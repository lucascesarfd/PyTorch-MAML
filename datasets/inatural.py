...
class INat2017(Dataset):
    def __init__(self, root_path, split="train", image_size=84, normalization=True, transform=None):
        super(INat2017, self).__init__()
        split_dict = {
            "train": "train",  # standard train
        ...
        with open(split_file, "r") as f:
            pairs = [x.strip().split(",") for x in f.readlines() if x.strip() != ""]
        ...
        if normalization:
            self.norm_params = {"mean": [0.4905, 0.4961, 0.4330], "std": [0.1737, 0.1713, 0.1779]}
        else:
            self.norm_params = {"mean": [0.0, 0.0, 0.0], "std": [1.0, 1.0, 1.0]}
...
class MetaINat2017(INat2017):
    def __init__(
        ...
        super(MetaINat2017, self).__init__(root_path, split, image_size, normalization, transform)
