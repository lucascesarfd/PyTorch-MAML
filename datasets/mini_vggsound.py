...
class MiniVGGSound(Dataset):
    def __init__(self, root_path, split="train", image_size=(128, 130), normalization=True, transform="sound"):
        ...
        data = [np.expand_dims(np.array(x), axis=-1) for x in data]
        ...
        self.norm_params = {"mean": [0.0], "std": [1.0]}

        self.transform = get_transform(transform, image_size, self.norm_params)

        def convert_raw(x):
            mean = torch.tensor(self.norm_params["mean"]).view(1, 1, 1).type_as(x)
            std = torch.tensor(self.norm_params["std"]).view(1, 1, 1).type_as(x)
            return x * std + mean
...
class MetaMiniVGGSound(MiniVGGSound):
    def __init__(
        ...
        super(MetaMiniVGGSound, self).__init__(
            root_path, split, image_size, normalization, transform
        )
